...
export async function performSync(
  ...
  try {
    ...
    // Delete from server any card that no longer exists locally
    const localKeySet = new Set(localCards.map((c) => `${c.word}:${c.cardType}`));
    const serverOnlyCards = serverCards.filter((c) => !localKeySet.has(`${c.word}:${c.cardType}`));
    for (const c of serverOnlyCards) {
    ...
    // ── 5 & 6. Deck words (Main Deck + My Words) ─────────────────────────────
    try {
      const { getWordsInDeck } = await import("./deckStore");
      const [mainDeckWords, myVocabWords] = await Promise.all([
        getWordsInDeck(MAIN_DECK_ID),
        getWordsInDeck(MY_VOCAB_ID),
      ]);

      // Push local additions
      const allDeckEntries = [
        ...mainDeckWords.map((word: string) => ({ storyId: 0, word, addedAt: now, updatedAt: now })),
        ...myVocabWords.map((word: string) => ({ storyId: -1, word, addedAt: now, updatedAt: now })),
      ];
      if (allDeckEntries.length > 0) {
        await utils.client.sync.pushStoryDeckWords.mutate(allDeckEntries);
      }

      // Pull server list
      const serverDeckWords = await utils.client.sync.pullStoryDeckWords.query();

      // Compute deletions: words the server has that local no longer has
      const localMainSet = new Set(mainDeckWords);
      const localMyVocabSet = new Set(myVocabWords);
      const toDelete = serverDeckWords.filter((item) => {
        if (item.storyId === 0)  return !localMainSet.has(item.word);
        if (item.storyId === -1) return !localMyVocabSet.has(item.word);
        return false;
      });
      for (const item of toDelete) {
        await utils.client.sync.deleteStoryDeckWord.mutate({ storyId: item.storyId, word: item.word });
      }

      // Apply server additions that aren't local yet
      for (const item of serverDeckWords) {
        if (item.storyId === 0  && !localMainSet.has(item.word))    await addWordToDeck(MAIN_DECK_ID, item.word);
        if (item.storyId === -1 && !localMyVocabSet.has(item.word)) await addWordToDeck(MY_VOCAB_ID,  item.word);
      }
    } catch (e) {
