...
export default function Settings() {
  ...
  const handleReset = async () => {
    await clearAllCards();
    await clearAllDecks();
    // Clear all mashang_ localStorage keys
    const toRemove: string[] = [];
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (k && k.startsWith("mashang_")) toRemove.push(k);
    }
    toRemove.forEach((k) => localStorage.removeItem(k));
    setCardCount(0);
    // Also wipe server data if authenticated so re-sync doesn't restore old cards
    if (isAuthenticated) {
      try {
        await utils.client.sync.resetAllData.mutate();
      } catch (e) {
        console.warn("[Settings] Server reset failed:", e);
      }
    }
          ...
          <AccordionContent className="space-y-4 pb-5">
            ...
            {!settings.unlimitedReviews && (
              <>
                <div className="flex items-center justify-between">
                  <div>
                    <Label>Daily new word cap</Label>
                    <p className="text-xs text-muted-foreground">{settings.newWordCap} new words</p>
                  </div>
                  <div className="w-36">
                    <Slider min={1} max={50} step={1}
                      value={[settings.newWordCap]}
                      onValueChange={([v]) => updateSettings({ newWordCap: v })}
                    />
                  </div>
                </div>
