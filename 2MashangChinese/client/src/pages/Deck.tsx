...
function ReviewSession({
  ...
  // ── Build the queue ────────────────────────────────────────────────────────
  // Shuffle once and share the result between the queue and the persistence ref.
  // This avoids the double-shuffle bug where buildQueue() and originalQueueRef
  // independently shuffled, producing different orderings.
  function shuffleWithAntiAdjacency(input: FlashCard[]): FlashCard[] {
  ...
  // Compute the initial queue exactly once on mount.
  // If resuming a session, restore from the persisted order.
  // If fresh, shuffle cards and record the order for persistence.
  const initialQueueRef = useRef<FlashCard[]>((() => {
    ...
    return shuffleWithAntiAdjacency(cards);
  })());

  // Build original queue for persistence — use the already-shuffled order
  const originalQueueRef = useRef<CardKey[]>(
    initialSession?.originalQueue ??
    initialQueueRef.current.map((c) => ({ word: c.word, cardType: c.cardType }))
  );
...
export default function Deck() {
  ...
  // ── Memoized due cards ────────────────────────────────────────────────────
  // Recomputes only when allCards, deckWordSets, selectedDeckIds, or settings change.
  // Keeps New vs Review counts consistent with the button count.
  const filteredDueCards = useMemo(() => {
    ...
    // Apply new word cap separately from review cap
    const newCards = due.filter((c) => c.state === 0);
    const reviewCards = due.filter((c) => c.state !== 0);

    const cappedNew = settings.unlimitedReviews
      ? newCards
      : newCards.slice(0, settings.newWordCap > 0 ? settings.newWordCap : newCards.length);

    const cappedReviews = settings.unlimitedReviews
      ? reviewCards
      : settings.dailyReviewCap > 0
        ? reviewCards.slice(0, settings.dailyReviewCap)
        : reviewCards;

    // Re-merge: new cards first, then reviews (sorted by dueDate within each group)
    due = [...cappedNew, ...cappedReviews];
    ...
    return due;
  }, [allCards, deckWordSets, selectedDeckIds, settings]);

  // ── Stats scoped to selected decks ───────────────────────────────────────
  // These counts match what the Review button shows, so nothing looks out of sync.
  const { uniqueWords, newWords, learningWords, reviewWords } = useMemo(() => {
    const allowedWords = new Set<string>();
    Array.from(selectedDeckIds).forEach((id) => {
      Array.from(deckWordSets[id] ?? []).forEach((w) => allowedWords.add(w));
    });
