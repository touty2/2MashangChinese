...
export const syncRouter = router({
  ...
  deleteStoryDeckWord: publicProcedure
    .input(z.object({ storyId: z.number().int(), word: z.string() }))
    .mutation(async ({ input, ctx }) => {
      const userId = await requireUser(ctx);
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      await db
        ...
        .where(
          and(
            eq(storyDeckWords.userId, userId),
            eq(storyDeckWords.storyId, input.storyId),
            eq(storyDeckWords.word, input.word)
          )
        );
      return { success: true };
    }),
  ...
  resetAllData: publicProcedure.mutation(async ({ ctx }) => {
    ...
    // Delete all user data in order (but keep the user account itself)
    ...
    // NOTE: do NOT delete the users row — that is only for deleteAccount
